...
const HomeData = () => {
  ...
  return (
    <div className="min-h-[100vh] bg-white md:mt-[9vh] mt-[2vh] object-cover w-[100vw] flex gap-[4vh] justify-center py-[4vh] flex-wrap text-[#fff] relative">
    <div className="grid md:grid-cols-4 grid-cols-2 gap-[2vw] text-black transition-all ease-in-out">
      {data.map((e, index) => {
        return (
          <div key={index}>
            <div className="md:min-h-[70vh] h-[53vh] shadow-xl flex flex-col items-center gap-[4vh] w-[48vw] md:mt-4 mt-6 md:w-[20vw] overflow-hidden relative border-[#0000001b] border-[1px]">
              <div
              ...
              >
                {e.image ? (
                  <img
                  loading="lazy"
                    className="md:h-[32vh] h-[22vh] cursor-pointer md:w-[20vw] w-[100%] object-cover hover:scale-[1.1] hover:rounded-b-[6px] transition duration-300 ease-out"
                    src={e.image}
                  />
                ) : (
                  <img
                ...
                {e.stock || e.discountPercentage ? (
                  <div className="flex flex-col gap-1 absolute top-0 -left-6 md:left-0">
              ...
              <div className="flex flex-col w-full px-1 md:p-2 md:gap-y-[1vh] gap-y-[0.3vh]">
                ...
                {e.category ? (
                  <p className="font-semibold md:text-sm text-xs">
                    <b className="md:text-md text-xs">{e.category}</b>
                  </p>
                ) : (
                  ""
                )}
                ...
                {e.discount ? (
                  <p className="flex items-center gap-1 font-black md:mt-4 mt-2 md:text-sm text-xs">
                    Price:
                ...
                {e.rating && (
                  <p className="font-semibold md:text-sm text-xs">
                    <b className="md:text-md text-xs">Ratings:</b> ⭐
                    {e.rating.rate}/5
                  </p>
                )}
                ...
                {e.color ? (
                  <p className="font-semibold md:text-sm text-xs">
                    <b className="md:text-md text-xs">Color:</b>{" "}
                    {e.color}
                  </p>
                ) : (
                  ""
                ...
                {e.model ? (
                  <p className="font-semibold md:text-sm text-xs">
                    <b className="md:text-md text-xs">Model:</b>{" "}
                    {e.model.slice(0, 29)}
                  </p>
                ) : (
                  ""
              ...
              {cart.some((item) => item.title === e.title) ? (
                <Link to="/cart" className="w-full">
                  <button onClick={goToCart} className="bg-[#dc2626] hover:text-[2.3vh] transition-all ease-out duration-[0.1s] hover:bg-[#ff4b4b] w-[100%] p-[1vh] text-white text-center font-semibold md:text-base text-sm absolute bottom-0">
                        Go to Cart{" "}
